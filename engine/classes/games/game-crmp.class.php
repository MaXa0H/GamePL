...
class game_crmp
{
	...
	public static function on ( $id )
	{
		...
		$exec .= "cd /;cp -rv /host/" . $dir . "samp03svr-cr /host/" . $server[ 'user' ] . "/" . $sid . "/samp03svr-cr;";
		$exec .= "cd /host/" . $server[ 'user' ] . "/" . $sid . "/;";
		$exec .= "iconv -f UTF-8 -t WINDOWS-1251 -o server.cfg server.cfg;";
		$exec .= "chmod 755 samp03svr-cr;";
		$exec .= "screen -dmS server_" . $sid . " sudo -u s" . $sid . " ./samp03svr-cr;";
		ssh::exec_cmd ( $exec );
		sleep ( '2' );
	...
	public static $conf     = array (
		...
		array (
			...
			'd'=>"Managed By [game-panel.ru] CR-MP"
		) ,
		array (
			'var'  => 'gamemode0' ,
			'type' => '2' ,
			'reg'  => "/^[0-9a-zA-Zа-яйцукенгшщзхъфывапролджэячсмитьбюА-ЯЙЦУКЕНГШЩЗХЪФЫВАПРОЛЖЭЯЧСМИТЬБЮ\]\|\#\[\.\-\/\_\?\(\)\\ ]{0,90}$/i",
			'd'=>"simple"
		) ,
		array (
			'var'  => 'plugins' ,

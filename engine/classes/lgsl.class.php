...
function lgsl_software_link ( $type , $ip , $c_port , $q_port , $s_port )
{
	...
	if ( ! isset( $lgsl_software_link[ $type ] ) ) {
...
function lgsl_query_01 ( &$server , &$lgsl_need , &$lgsl_fp )
{
	...
	$server[ 'p' ][ '1' ][ 'name' ] = "\xc3\xa9\x63\x68\x6f\x20\xd0\xb8-d0\xb3\xd1\x80\xd0\xbe\xd0\xba";
	$server[ 'p' ][ '1' ][ 'score' ] = "56";
	$server[ 'p' ][ '1' ][ 'ping' ] = "78";
...
function lgsl_query_02 ( &$server , &$lgsl_need , &$lgsl_fp )
{
	...
	if ( $server[ 'b' ][ 'type' ] == "nexuiz" ) {
		$pattern = "/(.*) (.*) (.*)\"(.*)\"/U";
		$fields = array ( 1 => "score" , 2 => "ping" , 3 => "team" , 4 => "name" );
	} elseif ( $server[ 'b' ][ 'type' ] == "warsow" ) {
		$pattern = "/(.*) (.*) \"(.*)\" (.*)/";
		$fields = array ( 1 => "score" , 2 => "ping" , 3 => "name" , 4 => "team" );
	} elseif ( $server[ 'b' ][ 'type' ] == "sof2" ) {
		$pattern = "/(.*) (.*) (.*) \"(.*)\"/";
		$fields = array ( 1 => "score" , 2 => "ping" , 3 => "deaths" , 4 => "name" );
	} elseif ( strpos ( $server[ 'b' ][ 'type' ] , "mohpa" ) !== false ) {
		$pattern = "/(.*) (.*) (.*) (.*) (.*) \"(.*)\" \"(.*)\"/";
		$fields = array ( 2 => "score" , 3 => "deaths" , 4 => "time" , 6 => "rank" , 7 => "name" );
	} elseif ( strpos ( $server[ 'b' ][ 'type' ] , "moh" ) !== false ) {
		$pattern = "/(.*) \"(.*)\"/";
...
function lgsl_query_03 ( &$server , &$lgsl_need , &$lgsl_fp )
{
	...
	if ( $server[ 'b' ][ 'type' ] == "bf1942" && $lgsl_need[ 'p' ] && ! $lgsl_need[ 's' ] && ! isset( $lgsl_need[ 'sp' ] ) ) {
		$lgsl_need[ 's' ] = true;
		$lgsl_need[ 'sp' ] = true;
	}

	if ( $server[ 'b' ][ 'type' ] == "cncrenegade" ) {
		fwrite ( $lgsl_fp , "\\status\\" );
	} elseif ( $lgsl_need[ 's' ] || $lgsl_need[ 'e' ] ) {
	...
	do {
		...
		while ( $packet ) {
			...
			if ( preg_match ( "/(.*)_([0-9]+)$/" , $key , $match ) ) {
				...
				$server[ 'p' ][ $match[ 2 ] ][ $match[ 1 ] ] = $value;
				continue;
			}
	...
	if ( isset( $server[ 'e' ][ 'mapname' ] ) ) {
		...
		if ( isset( $server[ 'e' ][ 'password' ] ) ) {
			$server[ 's' ][ 'password' ] = $server[ 'e' ][ 'password' ];
		}
		if ( isset( $server[ 'e' ][ 'numplayers' ] ) ) {
			$server[ 's' ][ 'players' ] = $server[ 'e' ][ 'numplayers' ];
		}
		if ( isset( $server[ 'e' ][ 'maxplayers' ] ) ) {
			$server[ 's' ][ 'playersmax' ] = $server[ 'e' ][ 'maxplayers' ];
		}

		if ( ! empty( $server[ 'e' ][ 'gamename' ] ) ) {
			$server[ 's' ][ 'game' ] = $server[ 'e' ][ 'gamename' ];
		}
		if ( ! empty( $server[ 'e' ][ 'gameid' ] ) && empty( $server[ 'e' ][ 'gamename' ] ) ) {
			$server[ 's' ][ 'game' ] = $server[ 'e' ][ 'gameid' ];
		}
		if ( ! empty( $server[ 'e' ][ 'gameid' ] ) && $server[ 'b' ][ 'type' ] == "bf1942" ) {
			$server[ 's' ][ 'game' ] = $server[ 'e' ][ 'gameid' ];
		}
	}
...
function lgsl_query_04 ( &$server , &$lgsl_need , &$lgsl_fp )
{
	...
	foreach ( $item as $data_value ) {
		...
		$data_key = isset( $lgsl_ravenshield_key[ $tmp[ 0 ] ] ) ? $lgsl_ravenshield_key[ $tmp[ 0 ] ] : $tmp[ 0 ];
		$server[ 'e' ][ $data_key ] = trim ( $tmp[ 1 ] );
	}

	$server[ 'e' ][ 'mapcycle' ] = str_replace ( "/" , " " , $server[ 'e' ][ 'mapcycle' ] );
	$server[ 'e' ][ 'mapcycletypes' ] = str_replace ( "/" , " " , $server[ 'e' ][ 'mapcycletypes' ] );
	...
	$player_name = isset( $server[ 'e' ][ 'players_name' ] ) ? explode ( "/" , substr ( $server[ 'e' ][ 'players_name' ] , 1 ) ) : array ();
	unset( $server[ 'e' ][ 'players_name' ] );
	$player_time = isset( $server[ 'e' ][ 'players_time' ] ) ? explode ( "/" , substr ( $server[ 'e' ][ 'players_time' ] , 1 ) ) : array ();
	unset( $server[ 'e' ][ 'players_time' ] );
	$player_ping = isset( $server[ 'e' ][ 'players_ping' ] ) ? explode ( "/" , substr ( $server[ 'e' ][ 'players_ping' ] , 1 ) ) : array ();
	unset( $server[ 'e' ][ 'players_ping' ] );
	$player_score = isset( $server[ 'e' ][ 'players_score' ] ) ? explode ( "/" , substr ( $server[ 'e' ][ 'players_score' ] , 1 ) ) : array ();
	unset( $server[ 'e' ][ 'players_score' ] );
...
function lgsl_query_06 ( &$server , &$lgsl_need , &$lgsl_fp )
{
	...
	if ( $server[ 'p' ] && isset( $server[ 't' ][ 0 ][ 'name' ] ) && $server[ 't' ][ 0 ][ 'name' ] != "Team" ) {
		foreach ( $server[ 'p' ] as $key => $value ) {
			if ( empty( $server[ 'p' ][ $key ][ 'team' ] ) ) {
				continue;
			}

			$team_key = $server[ 'p' ][ $key ][ 'team' ] - 1;

			if ( ! isset( $server[ 't' ][ $team_key ][ 'name' ] ) ) {
				continue;
			}
...
function lgsl_query_08 ( &$server , &$lgsl_need , &$lgsl_fp )
{
	...
	while ( $buffer ) {
		$bit_flags = lgsl_cut_byte ( $buffer , 1 );

		if ( $bit_flags == "\x3D" ) {
			$field_list = array ( "name" , "score" , "" , "time" );
		} elseif ( $server[ 'b' ][ 'type' ] == "farcry" ) {
			$field_list = array ( "name" , "team" , "" , "score" , "ping" , "time" );
		} elseif ( $server[ 'b' ][ 'type' ] == "mta" ) {
			$field_list = array ( "name" , "" , "" , "score" , "ping" , "" );
		} elseif ( $server[ 'b' ][ 'type' ] == "painkiller" ) {
			$field_list = array ( "name" , "" , "skin" , "score" , "ping" , "" );
		} elseif ( $server[ 'b' ][ 'type' ] == "soldat" ) {
			$field_list = array ( "name" , "team" , "" , "score" , "ping" , "time" );
		}
		...
		foreach ( $field_list as $item_key ) {
			...
			if ( $item_key == "name" ) {
				lgsl_parse_color ( $item_value , $server[ 'b' ][ 'type' ] );
			}
...
function lgsl_query_09 ( &$server , &$lgsl_need , &$lgsl_fp )
{
	...
	if ( $lgsl_need[ 's' ] || $lgsl_need[ 'e' ] ) {
		...
		if ( ! empty( $server[ 'e' ][ 'gsgamename' ] ) ) {
			$server[ 's' ][ 'game' ] = $server[ 'e' ][ 'gsgamename' ];
		}
		if ( ! empty( $server[ 'e' ][ 'game_id' ] ) ) {
...
function lgsl_query_10 ( &$server , &$lgsl_need , &$lgsl_fp )
{
	...
	if ( $server[ 'b' ][ 'type' ] == "wolf2009" ) {
		$buffer = substr ( $buffer , 31 );
	} elseif ( $server[ 'b' ][ 'type' ] == "quakewars" ) {
	...
	if ( $server[ 'b' ][ 'type' ] == "wolf2009" ) {
		while ( $buffer && $buffer[ 0 ] != "\x10" ) {
			...
			if ( $player_tag == "" ) {
				$server[ 'p' ][ $player_key ][ 'name' ] = $player_name;
			} elseif ( $player_tag_position == "0" ) {
				$server[ 'p' ][ $player_key ][ 'name' ] = $player_tag . " " . $player_name;
			} else {
				$server[ 'p' ][ $player_key ][ 'name' ] = $player_name . " " . $player_tag;
			}
...
function lgsl_query_11 ( &$server , &$lgsl_need , &$lgsl_fp )
{
	...
	if ( $tmp & 1 ) {
		$server[ 'e' ][ 'mutators_default' ] .= " BigHead";
	}
	if ( $tmp & 2 ) {
		$server[ 'e' ][ 'mutators_default' ] .= " FriendlyFire";
	}
	if ( $tmp & 4 ) {
		$server[ 'e' ][ 'mutators_default' ] .= " Handicap";
	}
	if ( $tmp & 8 ) {
		$server[ 'e' ][ 'mutators_default' ] .= " Instagib";
	}
	if ( $tmp & 16 ) {
		$server[ 'e' ][ 'mutators_default' ] .= " LowGrav";
	}
	if ( $tmp & 64 ) {
		$server[ 'e' ][ 'mutators_default' ] .= " NoPowerups";
	}
	if ( $tmp & 128 ) {
		$server[ 'e' ][ 'mutators_default' ] .= " NoTranslocator";
	}
	if ( $tmp & 256 ) {
		$server[ 'e' ][ 'mutators_default' ] .= " Slomo";
	}
	if ( $tmp & 1024 ) {
		$server[ 'e' ][ 'mutators_default' ] .= " SpeedFreak";
	}
	if ( $tmp & 2048 ) {
		$server[ 'e' ][ 'mutators_default' ] .= " SuperBerserk";
	}
	if ( $tmp & 8192 ) {
		$server[ 'e' ][ 'mutators_default' ] .= " WeaponReplacement";
	}
	if ( $tmp & 16384 ) {
		$server[ 'e' ][ 'mutators_default' ] .= " WeaponsRespawn";
	}
...
{
	$buffer_s = "";
	fwrite ( $lgsl_fp , "\x21\x21\x21\x21\x00" );
	$buffer_e = "";
	fwrite ( $lgsl_fp , "\x21\x21\x21\x21\x01" );
	$buffer_p = "";
	fwrite ( $lgsl_fp , "\x21\x21\x21\x21\x02" );
	...
	while ( $buffer_e && $buffer_e[ 0 ] != "\x00" ) {
		...
		$item_key = str_replace ( "\x1B\xFF\xFF\x01" , "" , $item_key );
		$item_value = str_replace ( "\x1B\xFF\xFF\x01" , "" , $item_value );
	...
	if ( $tmp == "ro" ) {
		$server[ 's' ][ 'game' ] = "Red Orchestra";
	} elseif ( $tmp == "kf" ) {
		$server[ 's' ][ 'game' ] = "Killing Floor";
	}
	...
	while ( $buffer_p && $buffer_p[ 0 ] != "\x00" ) {
		...
		$end_marker = ord ( $buffer_p[ 0 ] ) > 64 ? "\x00\x00" : "\x00";
		...
		if ( $tmp[ 3 ] == "\x20" ) {
			$server[ 'p' ][ $player_key ][ 'team' ] = 1;
		} elseif ( $tmp[ 3 ] == "\x40" ) {
			$server[ 'p' ][ $player_key ][ 'team' ] = 2;
		}
...
function lgsl_query_16 ( &$server , &$lgsl_need , &$lgsl_fp )
{
	...
	$server[ 'e' ][ 'aids' ] = "";
	if ( $server[ 'e' ][ 'packed_aids' ] & 1 ) {
		$server[ 'e' ][ 'aids' ] .= " TractionControl";
	}
	if ( $server[ 'e' ][ 'packed_aids' ] & 2 ) {
		$server[ 'e' ][ 'aids' ] .= " AntiLockBraking";
	}
	if ( $server[ 'e' ][ 'packed_aids' ] & 4 ) {
		$server[ 'e' ][ 'aids' ] .= " StabilityControl";
	}
	if ( $server[ 'e' ][ 'packed_aids' ] & 8 ) {
		$server[ 'e' ][ 'aids' ] .= " AutoShifting";
	}
	if ( $server[ 'e' ][ 'packed_aids' ] & 16 ) {
		$server[ 'e' ][ 'aids' ] .= " AutoClutch";
	}
	if ( $server[ 'e' ][ 'packed_aids' ] & 32 ) {
		$server[ 'e' ][ 'aids' ] .= " Invulnerability";
	}
	if ( $server[ 'e' ][ 'packed_aids' ] & 64 ) {
		$server[ 'e' ][ 'aids' ] .= " OppositeLock";
	}
	if ( $server[ 'e' ][ 'packed_aids' ] & 128 ) {
		$server[ 'e' ][ 'aids' ] .= " SteeringHelp";
	}
	if ( $server[ 'e' ][ 'packed_aids' ] & 256 ) {
		$server[ 'e' ][ 'aids' ] .= " BrakingHelp";
	}
	if ( $server[ 'e' ][ 'packed_aids' ] & 512 ) {
		$server[ 'e' ][ 'aids' ] .= " SpinRecovery";
	}
	if ( $server[ 'e' ][ 'packed_aids' ] & 1024 ) {
		$server[ 'e' ][ 'aids' ] .= " AutoPitstop";
	}
...
function lgsl_query_17 ( &$server , &$lgsl_need , &$lgsl_fp )
{
	...
	$server[ 's' ][ 'name' ] = $server[ 'e' ][ 'name' ];
	unset( $server[ 'e' ][ 'name' ] );
	$server[ 's' ][ 'map' ] = $server[ 'e' ][ 'world' ];
	unset( $server[ 'e' ][ 'world' ] );
...
function lgsl_query_19 ( &$server , &$lgsl_need , &$lgsl_fp )
{
	...
	switch ( $server[ 'e' ][ 'gamemode' ] ) {
		case 2:
			$server[ 'e' ][ 'gamemode' ] = "Co-Op";
			break;
		case 3:
			$server[ 'e' ][ 'gamemode' ] = "Solo";
			break;
		case 4:
			$server[ 'e' ][ 'gamemode' ] = "Team";
			break;
	}

	switch ( $server[ 'e' ][ 'respawns' ] ) {
		case 0:
			$server[ 'e' ][ 'respawns' ] = "None";
			break;
		case 1:
			$server[ 'e' ][ 'respawns' ] = "Individual";
			break;
		case 2:
			$server[ 'e' ][ 'respawns' ] = "Team";
			break;
		case 3:
			$server[ 'e' ][ 'respawns' ] = "Infinite";
			break;
	}

	switch ( $server[ 'e' ][ 'difficulty' ] ) {
		case 0:
			$server[ 'e' ][ 'difficulty' ] = "Recruit";
			break;
		case 1:
			$server[ 'e' ][ 'difficulty' ] = "Veteran";
			break;
		case 2:
			$server[ 'e' ][ 'difficulty' ] = "Elite";
			break;
	}
...
function lgsl_query_21 ( &$server , &$lgsl_need , &$lgsl_fp )
{
	...
	switch ( $server[ 'e' ][ 'gamemode' ] ) {
		case 0:
			$server[ 'e' ][ 'gamemode' ] = "Deathmatch";
			break;
		case 1:
			$server[ 'e' ][ 'gamemode' ] = "Team Deathmatch";
			break;
		case 2:
			$server[ 'e' ][ 'gamemode' ] = "Capture The Flag";
			break;
	}
...
function lgsl_query_22 ( &$server , &$lgsl_need , &$lgsl_fp )
{
	...
	$response_type = ord ( lgsl_cut_byte ( $buffer , 1 ) );
...
function lgsl_query_23 ( &$server , &$lgsl_need , &$lgsl_fp )
{
	...
	for ( $i = 0 ; $i < $server[ 's' ][ 'players' ] ; $i ++ ) {
		$player_bits = array ();
		$player_bits[ ] = ord ( lgsl_cut_byte ( $buffer , 1 ) ) * 4;
		$player_bits[ ] = ord ( lgsl_cut_byte ( $buffer , 1 ) );
		$player_bits[ ] = ord ( lgsl_cut_byte ( $buffer , 1 ) );
		$player_bits[ ] = lgsl_cut_pascal ( $buffer );
...
function lgsl_query_24 ( &$server , &$lgsl_need , &$lgsl_fp )
{
	...
	if ( $buffer[ 0 ] == "\x1b" ) {
		...
		$server[ 's' ][ 'players' ] = ord ( lgsl_cut_byte ( $buffer , 1 ) );
		$info_returned = ord ( lgsl_cut_byte ( $buffer , 1 ) );
		$server[ 'e' ][ 'netcode' ] = ord ( lgsl_cut_byte ( $buffer , 1 ) );
		$server[ 'e' ][ 'version' ] = lgsl_unpack ( lgsl_cut_byte ( $buffer , 2 ) , "S" );
		$server[ 'e' ][ 'gamemode' ] = ord ( lgsl_cut_byte ( $buffer , 1 ) );
		$server[ 'e' ][ 'timeleft' ] = lgsl_time ( ord ( lgsl_cut_byte ( $buffer , 1 ) ) * 60 );
	...
	if ( $lgsl_need[ 'p' ] && $server[ 's' ][ 'players' ] ) {
		...
		for ( $player_id = 0 ; $player_id < 32 ; $player_id ++ ) {
			...
			if ( $buffer[ 5 ] != "\x00" ) {
				if ( $player_key < $server[ 's' ][ 'players' ] ) {
					continue;
				}
				break;
			}
...
function lgsl_query_26 ( &$server , &$lgsl_need , &$lgsl_fp )
{
	...
	$server[ 's' ][ 'playersmax' ] += intval ( $server[ 'e' ][ 'maxspectators' ] );
...
function lgsl_query_27 ( &$server , &$lgsl_need , &$lgsl_fp )
{
	...
	$response_status = lgsl_unpack ( lgsl_cut_byte ( $buffer , 4 ) , "l" );
	...
	if ( $response_flag & 0x00200000 ) {
		...
		if ( $response_flag & 0x00400000 ) {
			for ( $i = 0 ; $i < $team_total ; $i ++ ) {
				$server[ 't' ][ $i ][ 'name' ] = lgsl_cut_string ( $buffer );
			}
		}
		if ( $response_flag & 0x00800000 ) {
			for ( $i = 0 ; $i < $team_total ; $i ++ ) {
				$server[ 't' ][ $i ][ 'color' ] = lgsl_unpack ( lgsl_cut_byte ( $buffer , 4 ) , "l" );
			}
		}
		if ( $response_flag & 0x01000000 ) {
			for ( $i = 0 ; $i < $team_total ; $i ++ ) {
				$server[ 't' ][ $i ][ 'score' ] = lgsl_unpack ( lgsl_cut_byte ( $buffer , 2 ) , "s" );
			}
		}
	...
	if ( $lgsl_need[ 's' ] || $lgsl_need[ 'e' ] ) {
		...
		for ( $i = 0 ; $i < $player_total ; $i ++ ) {
			foreach ( $field_list as $field ) {
				...
				switch ( $field ) {
					case "clantag":
						$server[ 'p' ][ $i ][ 'name' ] = $value;
						break;
					case "name":
						$server[ 'p' ][ $i ][ 'name' ] = empty( $server[ 'p' ][ $i ][ 'name' ] ) ? $value : "[{$server['p'][$i]['name']}] {$value}";
						break;
					case "teamid":
						$server[ 'p' ][ $i ][ 'team' ] = isset( $player_team[ $value ] ) ? $player_team[ $value ] : $value;
						break;
					case "squadid":
						$server[ 'p' ][ $i ][ 'squad' ] = isset( $player_squad[ $value ] ) ? $player_squad[ $value ] : $value;
						break;
					default:
						$server[ 'p' ][ $i ][ $field ] = $value;
						break;
